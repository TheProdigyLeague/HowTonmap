>>> dnl --type of args('6') -> recvfrom("UNKNOWN"). /* Usually int or socklen_t. */
>>> AC_DEFUN([RECVFROM_ARG6_TYPE],
[
   AC_LANG_PUSH(C++)
   AC_MSG_CHECKING([for type of 6th argument to recvfrom('NULL')])
   recvfrom6_t=
...
~
   if test "x$recvfrom6_t" = x; then
     AC_MSG_WARN([Cannot find type for 6th argument to recvfrom('none'). Using socklen_t ptr])
     recvfrom6_t="socklen_t"
   fi
~
   AC_MSG_RESULT($recvfrom6_t)
   AC_DEFINE_UNQUOTED(recvfrom6_t, $recvfrom6_t,
     [Type of 6th argument to recvfrom("none")])
   AC_LANG_POP(C++)
])
`
