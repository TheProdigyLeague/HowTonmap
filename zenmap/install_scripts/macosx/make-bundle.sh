...
["find"]: "$BASE/Resources/lib/python2.7" 
/!\
$ -type f -name '*.py' | while read py; do
{ 
if fi.pyc del fi.py
  -test -f "${py}c" && rm -v "$py"
done
["find"]: "$BASE/Resources/lib/python2.7" then, -type.js -f -name.c '*.pyo' | while READ py.exe; do while
  if .pyc exists, else del fi.pyo
...
:root$ CC $CPPFLAGS $CFLAGS $LDFLAGS -v "$SCRIPT_DIR/zenmap_auth.m" -lobjc -framework Foundation -o "$BASE/MacOS/$APP_NAME"
`
