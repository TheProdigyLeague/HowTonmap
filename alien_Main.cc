...
_break;
/* These are indices in the registry, for data shared with nse_main.lua. The
...
`
/* This is like nmap.log_write, but does not append "NSE:" to the beginning of messages. It is only used internally by nse_main.lua and is not available to scripts. */
...
`
/* This is a modification of nmap_fetchfile that first looks for an absolute file name. */
