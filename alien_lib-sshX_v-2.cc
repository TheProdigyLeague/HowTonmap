...
(tm)
    ~
$ bind for libssh2 \libs & note: "if is 1x1 -cor -betwix Function("libssh2") in --bind __"break;
$ while ssh2 /!\hand_shake -call @nsock.receiver throw, "result/!\▶eof" ERROR break;
$ -ssh --srv --brute _force | while --strict "ids" \firewall
extern "C" {
# include "libssh2.h"
};
# include "nse_lua.h"
...
* returns true on success and false otherwise userauth_password(state, username, password)
*/
